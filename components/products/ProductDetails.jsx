...
const ProductDetails = ({product}) => {
  ...
  // to display other product pictures
  const imgRef = useRef(null);
  const setImgPreview = (url) => {
    imgRef.current.src = url;
  };

  // to set all product pictures to same size
  useEffect(() => {
    const firstImg = document.querySelector('.product-img');
    if (firstImg) {
      const width = firstImg.offsetWidth;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-5">
              <aside>
                <div className="border border-gray-200 shadow-sm p-3 text-center rounded mb-5">
                  <img
                    ref={imgRef}
                    className="object-contain inline-block product-img"
                ...
                <div className="space-x-2 overflow-auto text-center whitespace-nowrap">
                  ...
                  <a className="inline-block border border-gray-200 p-1 rounded-md hover:border-blue-500 cursor-pointer" onClick={() => setImgPreview(img?.url)}>
                    <img
                      className="w-14 h-14"
                      src={img.url}
                      alt="Product title"
